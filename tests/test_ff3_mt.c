/**
 * @file test_ff3_mt.c
 * @brief Multi-threaded tests for FF3 algorithm
 * 
 * Tests:
...
/* Thread worker for FF3 encryption */
void* ff3_worker(void* arg) {
    ...
    int ret = FPE_CTX_init(ctx, FPE_MODE_FF3, FPE_ALGO_AES, 
...
static void run_ff3_mt_test(int num_threads, int ops_per_thread, 
    ...
    /* Create threads */
    for (int i = 0; i < num_threads; i++) {
        int ret = pthread_create(&threads[i], NULL, ff3_worker, &args[i]);
...
/* Test 5.25: Multiple thread counts */
void test_ff3_multiple_thread_counts(void) {
    ...
    printf("\n");
    printf("========================================\n");
    printf("FF3 Multi-Threading Performance Tests\n");
    printf("========================================\n");
    ...
    for (int i = 0; i < num_tests; i++) {
        ...
        run_ff3_mt_test(thread_counts[i], ops_per_thread, key, 128, 10, 10,
...
/* Test 5.26: Verify TPS scales with thread count */
void test_ff3_tps_scaling(void) {
    unsigned char key[16] = {
        0xEF, 0x43, 0x59, 0xD8, 0xD5, 0x80, 0xAA, 0x4F,
        0x7F, 0x03, 0x6D, 0x6F, 0x04, 0xFC, 0x6A, 0x94
    };
    ...
    printf("\n");
    printf("========================================\n");
    printf("FF3 TPS Scaling Verification\n");
    printf("========================================\n");
    printf("\n");
...
/* Test 5.27: Thread safety verification */
void test_ff3_thread_safety(void) {
    unsigned char key[16] = {
        0xEF, 0x43, 0x59, 0xD8, 0xD5, 0x80, 0xAA, 0x4F,
        0x7F, 0x03, 0x6D, 0x6F, 0x04, 0xFC, 0x6A, 0x94
    };
    ...
    run_ff3_mt_test(16, 1000, key, 128, 10, 10,
                   &total_ops, &elapsed_sec, &tps);
...
int main(void) {
    ...
    RUN_TEST(test_ff3_multiple_thread_counts);
    RUN_TEST(test_ff3_tps_scaling);
    RUN_TEST(test_ff3_thread_safety);
